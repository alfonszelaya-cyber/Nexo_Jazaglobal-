...
def register_identity(full_name, role_name, created_by_id):
    ...
    if role_name in ["ROOT", "DUEÑO"]:
        level = SecurityLevel.ROOT
    elif role_name in ["MANAGER", "GERENTE"]:
        level = SecurityLevel.MANAGER
    elif role_name in ["VENDEDOR", "CONTADOR"]:
    ...
    ledger_record("IDENTITY_CREATED", "OK",
                  {"id": new_id, "rol": role_name},
                  "BUNKER")
...
def report_security_incident(identity_id, incident_type):
    ...
    if lock_applied > 0:
        ledger_record("SECURITY_LOCK",
                      f"LEVEL_{lock_applied}",
    ...
    return {
        "current_lock": identity["lock_level"],
...
def execute_unlock(target_id, authorizer_id, method="MANUAL"):
    ...
    if current_lock == LockLevel.NONE:
        ...
        if admin_rank >= SecurityLevel.STAFF:
            authorized = True

    elif current_lock == LockLevel.L2_HARD:
        if admin_rank >= SecurityLevel.MANAGER:
            authorized = True

    elif current_lock == LockLevel.L3_CRIT:
        if admin_rank >= SecurityLevel.ROOT:
            authorized = True
...
if __name__ == "__main__":
    ...
    report_security_incident(uid, "INTRUSION_DETECTED")
